...
import { JobPersistenceUnavailableError, listRemoteJobs } from "@/lib/marketplace/job-api";
...
export function JobsBrowser() {
  ...
  useEffect(() => {
    const refresh = async () => {
      ...
      try {
        const remoteJobs = await listRemoteJobs();
        const jobsById = new Map(localJobs.map((job) => [job.id, job]));
        remoteJobs.forEach((remoteJob) => {
          const localJob = jobsById.get(remoteJob.id);
          if (!localJob || new Date(remoteJob.updatedAt).getTime() >= new Date(localJob.updatedAt).getTime()) {
            jobsById.set(remoteJob.id, remoteJob);
          }
        });
        setJobs([...jobsById.values()].sort((left, right) => right.updatedAt.localeCompare(left.updatedAt)));
        setStorageNotice(undefined);
      } catch (error) {
        setJobs(localJobs);
        setStorageNotice(error instanceof JobPersistenceUnavailableError ? "Server persistence is unavailable. Only local drafts are visible." : "The saved jobs service is unavailable. Showing the local fallback.");
      }
      setReady(true);
    };
      ...
      <main className="mx-auto max-w-7xl px-4 pb-28 pt-10 sm:px-6 sm:pt-12 lg:pt-20">
        <section className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
          <div className="max-w-3xl">
            ...
            <p className="mt-6 max-w-2xl text-lg leading-7 text-muted text-wrap-pretty">Server saved jobs and local drafts share one view. On chain jobs require durable server storage. Payment receipts and status changes appear only after a wallet or network response proves them.</p>
          </div>
        ...
        <section className="mt-12" aria-live="polite">
          {!ready ? (
            <div className="rounded-3xl border border-surface-border bg-surface px-6 py-16 text-center text-sm text-muted">Loading saved jobs</div>
          ) : jobs.length === 0 ? (
        ...
        <section className="mt-12 rounded-3xl border border-dashed border-surface-border bg-surface p-6 sm:p-8">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">Open a job to review its Altana spend cap, contract and token allowlists, expiration, and revoke intent. No session key or on chain revoke transaction is created by this UI yet.</p>
        </section>
      </main>
    </div>
  );
